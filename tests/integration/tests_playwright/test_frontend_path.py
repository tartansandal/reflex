...
def onload_redirect_with_prefix_app(
    ...
    # Set up environment for prefix serving
...
def test_onload_redirect(onload_redirect_app: AppHarness, page: Page):
    assert onload_redirect_app.frontend_url is not None

    base_url = onload_redirect_app.frontend_url.rstrip("/")
    page.goto(base_url)
...
def test_onload_redirect_with_prefix(
    onload_redirect_with_prefix_app: AppHarness, page: Page
):
    assert onload_redirect_with_prefix_app.frontend_url is not None

    base_url = onload_redirect_with_prefix_app.frontend_url.rstrip("/")
    base_url += "/prefix/"
    page.goto(base_url)
    expect(page.get_by_text("This is the index page!")).to_be_visible()
